        ...
        <main class="flex-1 flex flex-col h-screen overflow-y-auto bg-[#F9FAFB] hide-scrollbar">
            ...
            <div
                ...
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <h2 class="text-lg font-extrabold text-gray-900 leading-none mb-0.5">
                            {{ $canteen->name ?? 'Kantin' }}</h2>
                ...
                <div class="flex items-center gap-3">
                    <span id="statusLabel"
                        class="text-xs font-black tracking-widest {{ $canteen->is_open ? 'text-green-500' : 'text-gray-400' }}">
                        {{ $canteen->is_open ? 'MENERIMA PESANAN' : 'TUTUP SEMENTARA' }}
                    </span>
                    <div id="toggleTrack" onclick="toggleKantin()" class="toggle-track"
                        style="background-color: {{ $canteen->is_open ? '#22c55e' : '#d1d5db' }}; cursor:pointer;">
                        <span id="toggleThumb" class="toggle-thumb"
            ...
            @if(session('success'))
                <div id="flashSuccess"
                    class="mx-10 mt-6 px-5 py-3 bg-green-50 border border-green-100 rounded-2xl text-green-700 text-sm font-semibold flex items-center gap-2 transition-opacity duration-500">
            ...
            @if(session('error'))
                <div id="flashError"
                    class="mx-10 mt-6 px-5 py-3 bg-red-50 border border-red-100 rounded-2xl text-red-600 text-sm font-semibold flex items-center gap-2 transition-opacity duration-500">
            ...
            <div class="flex items-center px-10 mt-8 border-b border-gray-100">
                <button onclick="switchTab('masuk')" id="tabMasukBtn"
                    ...
                    Pesanan Masuk
                    <span id="badgeMasuk"
                        style="margin-left:8px; padding:2px 8px; background:#FF6900; color:white; border-radius:999px; font-size:10px; font-weight:900;">{{ $menungguVerifikasi->count() }}</span>
                </button>
                <button onclick="switchTab('diproses')" id="tabDiprosesBtn"
                    ...
                    Diproses
                    <span id="badgeDiproses"
                        style="margin-left:8px; padding:2px 8px; background:#e5e7eb; color:#9ca3af; border-radius:999px; font-size:10px; font-weight:900;">{{ $sedangDiproses->count() }}</span>
                </button>
            </div>
                            ...
                            <div class="card-order">
                                ...
                                <div class="flex justify-between items-start mb-5">
                                    <div class="flex items-center gap-3">
                                        ...
                                        @endif
                                        <div>
                                            <p class="text-[15px] font-black text-gray-800">
                                                {{ $order->customer_snapshot['name'] ?? '-' }}</p>
                                ...
                                <div class="flex justify-between items-center">
                                    <div>
                                        ...
                                        <p class="text-lg font-black text-[#FF6900]">Rp
                        ...
                        @foreach($sortedDiproses as $order)
                            <a href="{{ route('admin.pesanan.status', $order->_id) }}" class="card-order block"
                                ...
                                <div class="flex justify-between items-center pt-4 border-t border-gray-50">
                                    ...
                                    @endif
                                    <span class="btn-detail">Lihat Detail <i
                                            class="fa-solid fa-chevron-right text-[11px]"></i></span>
                                </div>
                            </a>
                        @endforeach
                    </div>
