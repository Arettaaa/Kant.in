...
@section('content')
    <div class="flex w-full h-screen bg-[#F9FAFB] overflow-hidden text-start">
        ...
        <aside
            ...
            <form method="POST" action="{{ route('logout') }}" class="mt-auto border-t border-gray-100 pt-6">
                @csrf
                <button type="submit"
                    class="w-full flex items-center gap-3 px-4 py-3 rounded-2xl text-[15px] font-bold text-gray-400 hover:text-red-500 hover:bg-red-50 transition-all">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <main class="flex-1 flex flex-col h-screen overflow-y-auto bg-[#F9FAFB]">
            ...
            <div class="sticky top-0 z-10 flex items-center justify-between px-10 py-5 bg-white border-b border-gray-100">
                ...
                @endphp
                <span
                    class="px-4 py-1.5 rounded-full text-[11px] font-black uppercase tracking-widest border {{ $s['class'] }}">
                    {{ $s['label'] }}
                </span>
            </div>
            ...
            @if(session('success'))
                <div
                    class="mx-10 mt-5 px-5 py-3 bg-green-50 border border-green-100 rounded-2xl text-green-700 text-sm font-semibold flex items-center gap-2">
                    <i class="fa-solid fa-circle-check text-green-500"></i> {{ session('success') }}
                </div>
            @endif
            @if(session('error'))
                <div
                    class="mx-10 mt-5 px-5 py-3 bg-red-50 border border-red-100 rounded-2xl text-red-600 text-sm font-semibold flex items-center gap-2">
                ...
                <div class="col-span-12 lg:col-span-7 space-y-6">
                    ...
                    <div class="bg-white rounded-[28px] p-6 shadow-sm border border-gray-100">
                        <div class="flex items-center gap-4">
                            ...
                            @else
                                <div
                                    class="w-14 h-14 rounded-full bg-gray-100 flex items-center justify-center text-gray-300 text-2xl">
                                    <i class="fa-solid fa-user"></i>
                                </div>
                            @endif
                            <div>
                                <p class="text-xl font-black text-gray-800">{{ $order->customer_snapshot['name'] ?? '-' }}
                                </p>
                                @if(!empty($order->customer_snapshot['phone']))
                                    <p class="text-sm text-gray-400 font-semibold mt-0.5">
                                        {{ $order->customer_snapshot['phone'] }}</p>
                                @endif
                                <div class="flex items-center gap-2 mt-1">
                                    @if(($order->delivery_details['method'] ?? '') === 'delivery')
                                        <span
                                            class="text-[10px] font-black bg-blue-50 text-blue-500 px-2.5 py-1 rounded-lg uppercase">Antar
                                            Kurir</span>
                                    @else
                                        <span
                                            class="text-[10px] font-black bg-purple-50 text-purple-500 px-2.5 py-1 rounded-lg uppercase">Ambil
                                            Sendiri</span>
                                    @endif
                                    @if(!empty($order->delivery_details['location_note']))
                                        <span class="text-[11px] text-gray-400 font-semibold"><i
                                                class="fa-solid fa-location-dot mr-1"></i>
                                            {{ $order->delivery_details['location_note'] }}</span>
                                    @endif
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="bg-white rounded-[28px] p-8 shadow-sm border border-gray-100">
                        ...
                        <div class="space-y-4">
                            @foreach($order->items as $item)
                                <div class="flex items-start gap-4 p-4 rounded-2xl bg-gray-50 border border-gray-100">
                                    <div
                                        class="w-10 h-10 rounded-xl bg-white flex items-center justify-center text-sm font-black text-[#FF6900] border border-gray-100 flex-shrink-0">
                                        {{ $item['quantity'] }}x
                                    </div>
                                    <div class="flex-1">
                        ...
                        @if($order->order_notes)
                            <div class="mt-5 p-4 bg-orange-50 rounded-2xl border border-orange-100">
                                <p class="text-[11px] font-black text-orange-400 uppercase tracking-widest mb-1"><i
                                        class="fa-solid fa-circle-info mr-1"></i> Catatan</p>
                        ...
                        <div class="mt-8 pt-6 border-t-2 border-dashed border-gray-100 space-y-3">
                            <div class="flex justify-between text-sm font-bold text-gray-400 uppercase">
                                <span>Subtotal</span>
                                <span class="text-gray-800 font-black">Rp
        ...
        : 'Gratis' }}
                            ...
                            <div class="flex justify-between items-center pt-4 border-t border-gray-100">
                                ...
                                <span class="text-2xl font-black text-[#FF6900]">Rp
                ...
                <div class="col-span-12 lg:col-span-5 space-y-6">
                    <div class="bg-white rounded-[28px] p-8 shadow-sm border border-gray-100">
                        ...
                        @if(!empty($order->payment['proof']))
                            <img src="{{ asset('storage/' . $order->payment['proof']) }}" onclick="toggleZoom(true, this.src)"
                                class="w-full rounded-2xl border border-gray-100 object-contain mb-6 cursor-pointer hover:opacity-90 transition-all"
                                style="max-height: 320px;" alt="Bukti Bayar">
                        @else
                            <div
                                class="w-full h-40 rounded-2xl bg-gray-50 border border-gray-100 flex items-center justify-center mb-6">
                                <p class="text-gray-300 text-sm font-semibold"><i class="fa-solid fa-image mr-1"></i> Tidak ada
                                    bukti bayar</p>
                            </div>
                        @endif
                        ...
                        <div class="text-sm text-gray-400 font-semibold mb-6 space-y-1">
                            <p><i class="fa-solid fa-calendar-check mr-1 text-gray-300"></i> Dipesan: <span
                                    class="text-gray-700 font-bold">{{ \Carbon\Carbon::parse($order->created_at)->timezone('Asia/Jakarta')->format('d M Y, H:i') }}
                                    WIB</span></p>
                            @if(!empty($order->payment['paid_at']))
                                <p><i class="fa-solid fa-money-bill-wave mr-1 text-gray-300"></i> Dibayar: <span
                                        class="text-gray-700 font-bold">{{ \Carbon\Carbon::parse($order->payment['paid_at'])->timezone('Asia/Jakarta')->format('d M Y, H:i') }}
                                        WIB</span></p>
                            @endif
                        </div>
                            ...
                            <div class="flex flex-col gap-3">
                                <form method="POST" action="{{ route('admin.pesanan.verify', $order->_id) }}">
                                    ...
                                    <button type="submit"
                                        ...
                                        <i class="fa-solid fa-check mr-1"></i> Verifikasi & Terima
                                    </button>
                                </form>
                                ...
                                <form method="POST" action="{{ route('admin.pesanan.reject', $order->_id) }}">
                                    ...
                                    <button type="submit"
                                        ...
                                        <i class="fa-solid fa-xmark mr-1"></i> Tolak Pembayaran
                                    </button>
                                </form>
                            </div>
                        @elseif($order->payment['status'] === 'paid' && $order->status === 'processing')
                            ...
                            <form method="POST" action="{{ route('admin.pesanan.status', $order->_id) }}">
                                ...
                                <button type="submit"
                                    ...
                                    <i class="fa-solid fa-utensils mr-1"></i> Tandai Siap Diambil
                                </button>
                            </form>
                        @else
    ...
    <div id="zoomModal"
        class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 transition-all">
        <button onclick="toggleZoom(false)" class="absolute top-6 right-6 text-white text-3xl hover:text-red-400">
            <i class="fa-solid fa-xmark"></i>
        </button>
        <img id="zoomImage" src="" class="max-w-full max-h-[90vh] rounded-2xl shadow-2xl" alt="Zoom Bukti Bayar">
    </div>

    @push('scripts')
        <script>
            function toggleZoom(show, src = '') {
            ...
            document.getElementById('zoomModal').addEventListener('click', function (e) {
                if (e.target === this) toggleZoom(false);
            });
        </script>
    @endpush
@endsection