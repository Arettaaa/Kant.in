...
<div class="flex w-full h-screen bg-[#F9FAFB] overflow-hidden text-start">
    ...
    <aside class="w-[240px] h-screen bg-white flex flex-col py-8 px-6 shadow-sm flex-shrink-0 z-20 border-r border-gray-100">
        ...
        <nav class="flex flex-col gap-2 flex-1 text-start">
            ...
            {{-- Riwayat Transaksi Jadi Oranye (AKTIF) --}}
    ...
    <main class="flex-1 flex flex-col h-screen overflow-y-auto bg-[#F9FAFB] hide-scrollbar text-start relative">
        ...
        {{-- Area Konten Grid Desktop --}}
        <div class="p-10">
            <div class="grid grid-cols-12 gap-8">
                
                {{-- Kolom Kiri: Ringkasan & Info Transaksi --}}
                        ...
                        <div class="w-full grid grid-cols-2 gap-4 pt-8 border-t border-gray-50 text-start">
                            <div class="bg-gray-50/50 p-4 rounded-[28px] border border-gray-100/50">
                                ...
                                <div class="flex items-center gap-2 font-black text-gray-800 text-sm">
                                    <i class="fa-solid fa-qrcode text-blue-500"></i> QRIS
                                </div>
                            </div>
                            <div class="bg-gray-50/50 p-4 rounded-[28px] border border-gray-100/50">
                ...
                <div class="col-span-12 lg:col-span-7">
                    <div class="bg-white rounded-[44px] p-10 border border-gray-100 shadow-sm space-y-10 text-start">
                        ...
                        <div id="detItemsList" class="space-y-8">
                            </div>
                        ...
                        <div class="pt-10 border-t-2 border-dashed border-gray-100 space-y-5">
                            <div class="flex justify-between text-xs font-bold text-gray-400 uppercase tracking-[0.1em]">
                                <span>Subtotal</span>
                                <span id="detSub" class="text-gray-900 font-black text-start">Rp 0</span>
                            </div>
                            <div class="flex justify-between text-xs font-bold text-gray-400 uppercase tracking-[0.1em]">
                                <span>PPN (11%)</span>
                                <span class="text-gray-900 font-black">Rp 0</span>
                            </div>
                            <div class="flex justify-between text-xs font-bold text-gray-400 uppercase tracking-[0.1em]">
                                <span>Biaya Aplikasi</span>
                                <span class="text-gray-900 font-black">Rp 2.000</span>
                            </div>
                            <div class="flex justify-between items-center pt-6 border-t border-gray-100">
                                <span class="text-lg font-black text-gray-900 uppercase tracking-tighter">Total Akhir</span>
                                <span id="detTotalFinal" class="text-3xl font-black text-[#FF6900]">Rp 0</span>
                            </div>
                        </div>
                    </div>
                </div>
...
<script>
    window.onload = function() {
        ...
        if(status === 'BATAL' || status === 'DIBATALKAN') {
            badge.className = "px-6 py-2 rounded-full bg-red-50 text-red-500 text-[10px] font-black uppercase border border-red-100";
        } else {
            badge.className = "px-6 py-2 rounded-full bg-green-50 text-[#22c55e] text-[10px] font-black uppercase border border-green-100";
        }

        const now = new Date();
        const dateStr = now.toLocaleDateString('id-ID', { year: 'numeric', month: 'short', day: 'numeric', timeZone: 'Asia/Jakarta' });
        const timeStr = now.toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit', hour12: false, timeZone: 'Asia/Jakarta' });
        document.getElementById('detWib').innerHTML = `${dateStr}<br><span class="text-gray-400 font-bold uppercase text-[9px]">${timeStr} WIB</span>`;
