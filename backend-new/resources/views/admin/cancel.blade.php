            ...
            <div class="bg-white rounded-[32px] p-8 shadow-sm border border-gray-100 w-full text-start">
                <div class="flex items-center justify-between mb-8 pb-6 border-b border-gray-50">
                    <div class="flex items-center gap-4">
                        <div id="custAvatar" class="w-14 h-14 rounded-full overflow-hidden border border-gray-100 flex items-center justify-center bg-orange-100 text-orange-500 font-black text-xl">
                            </div>
                        <div class="text-start">
                ...
                <div class="space-y-6 text-start">
                    ...
                    <div id="cancelItemList" class="space-y-4">
                        </div>
                </div>
            </div>
...
<script>
    window.onload = function() {
        // Ambil data dari URL (Link dari Riwayat)
        ...
        // Logika Waktu Real-Time WIB Indonesia
        const now = new Date();
        ...
        document.getElementById('detWaktu').innerText = `🕒 ${timeStr} WIB`;

        // Simulasi Menu khusus untuk Pesanan yang Batal (Biasanya 1 item)
        const container = document.getElementById('cancelItemList');
        let menuName = "Mie Goreng Seafood"; // Default
        let catatan = "Dibatalkan oleh pelanggan";

        if(name.includes('Megawati')) {
            menuName = "Mie Goreng Ayam";
            catatan = "Catatan: Tidak pedas";
        }
        ...
        const html = `
            <div class="flex items-start gap-4 p-4 bg-gray-50/50 rounded-2xl border border-gray-100">
                ...
                <div class="flex-1">
                    ...
                    <p class="text-xs text-red-500 font-bold mt-2 bg-red-50 inline-block px-3 py-1.5 rounded-xl italic">
                        ${catatan}
                    </p>
                </div>
            </div>`;
