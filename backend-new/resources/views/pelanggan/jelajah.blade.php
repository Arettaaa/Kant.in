...
<div class="flex w-full h-screen bg-[#F9FAFB] overflow-hidden">
    ...
    {{-- ======================== SIDEBAR MANUAL ======================== --}}
    @include('pelanggan.partials.sidebar', ['currentPath' => 'jelajah'])
        ...
        <div class="px-10 py-8 flex flex-col gap-6">
            ...
            <div class="flex items-center gap-1 bg-gray-100 p-1 rounded-2xl w-fit">
                <a href="{{ route('pelanggan.jelajah', ['tab' => 'makanan']) }}"
                    class="px-6 py-2.5 rounded-xl text-sm font-bold transition-all {{ $tab === 'makanan' ? 'text-white shadow-sm' : 'text-gray-400 hover:text-gray-600' }}"
                ...
                <a href="{{ route('pelanggan.jelajah', ['tab' => 'kantin']) }}"
                    class="px-6 py-2.5 rounded-xl text-sm font-bold transition-all {{ $tab === 'kantin' ? 'text-white shadow-sm' : 'text-gray-400 hover:text-gray-600' }}"
            ...
            {{-- CHIP FILTER BUKA/TUTUP --}}
            <div class="flex items-center gap-2">
                @foreach(['semua' => 'Semua', 'buka' => 'Buka', 'tutup' => 'Tutup'] as $value => $label)
...
<script>
    ...
    function submitSearchKantin() {
        const search = document.getElementById('searchKantinInput').value;
        const url = new URL(window.location.href);
        url.searchParams.set('tab', 'kantin');
