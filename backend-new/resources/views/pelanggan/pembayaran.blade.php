...
<div class="flex w-full h-screen bg-[#F9FAFB] overflow-hidden">
    ...
    <main class="flex-1 flex flex-col h-screen overflow-y-auto bg-[#F9FAFB]">
        <div class="flex flex-col gap-6 px-8 py-8 max-w-5xl w-full mx-auto">

            {{-- ===== HEADER (Dipindah ke atas agar span penuh) ===== --}}
            ...
            <div class="flex items-start gap-6 w-full">
                ...
                <div class="flex-1 flex flex-col gap-5">
                    ...
                    <div class="bg-white rounded-3xl shadow-sm border border-gray-100 p-6 flex flex-col items-center gap-5">
                        ...
                        {{-- QR Image --}}
                        <div class="qr-wrap">
                            <div class="w-52 h-52 rounded-2xl overflow-hidden border-4 border-white shadow-lg bg-white flex items-center justify-center p-2">
                                @if(!empty($canteen['qris_image']))
                                <img src="{{ $canteen['qris_image'] }}" alt="QRIS {{ $canteen['name'] ?? 'Kantin' }}" class="w-full h-full object-contain rounded-xl">
                                @else
                                {{-- Placeholder QR kalau belum ada --}}
                                <div class="w-full h-full flex flex-col items-center justify-center gap-2 bg-gray-50 rounded-xl">
                                    <i class="fa-solid fa-qrcode text-4xl text-gray-300"></i>
                                    <p class="text-[10px] font-bold text-gray-300 text-center leading-tight">QRIS belum<br>diatur</p>
                                </div>
                                @endif
                            </div>
                        </div>                   
                    ...
                    <div class="bg-white rounded-3xl shadow-sm border border-gray-100 p-6">
                        ...
                        <p class="text-xs text-gray-400 font-medium mb-4">Foto struk atau screenshot konfirmasi QRIS kamu</p>

                        <label for="fileInput" class="upload-area block border-2 border-dashed border-gray-200 rounded-2xl p-8 text-center">
                            <input type="file" id="fileInput" accept=".jpg,.jpeg,.png" class="hidden" onchange="handleFile(this)">
                ...
                {{-- ===== RIGHT COLUMN ===== --}}
                <div class="w-[300px] flex-shrink-0 flex flex-col gap-4">
                    ...
                    <div class="bg-white rounded-3xl shadow-sm border border-gray-100 p-6 flex flex-col gap-4">
                        ...
                        <div class="flex flex-col items-center gap-3">
                            ...
                            <div class="text-center">
                                ...
                                <p class="text-xs text-gray-400 font-semibold mt-0.5">{{ $canteen['name'] ?? 'Kantin' }}</p>
                            </div>
                        </div>
                            ...
                            <div class="flex items-start gap-2.5">
                                ...
                                <div class="flex-1 min-w-0">
                                    <div class="flex items-start justify-between gap-1">
                                        <p class="text-xs font-extrabold text-gray-800 leading-snug truncate max-w-[120px]">{{ $item['name'] }}</p>
                        ...
                        <div class="flex flex-col gap-2.5 text-sm">
                            <div class="flex items-center justify-between">
                                ...
                                <span class="font-bold text-gray-700">Rp {{ number_format($checkout['subtotal'], 0, ',', '.') }}</span>
                            </div>
                    ...
                    <div class="bg-white rounded-3xl shadow-sm border border-gray-100 p-5 flex flex-col gap-3">
                        ...
                        <div class="flex items-center gap-2.5">
                            @if($checkout['metode'] === 'kurir')
                            <div class="w-8 h-8 rounded-xl flex items-center justify-center flex-shrink-0" style="background-color:#FFF3E8;">
                    ...
                    {{-- Tombol Konfirmasi --}}
                    <button id="konfirmasiBtn" onclick="submitPembayaran()" class="konfirmasi-btn w-full py-4 rounded-2xl text-white font-extrabold text-sm shadow-lg flex items-center justify-center gap-2" style="background: linear-gradient(135deg, #d1d5db, #9ca3af);" disabled>
...
<div id="menungguModal" class="fixed inset-0 z-50 hidden items-center justify-center"
    ...
    <div class="modal-card bg-white rounded-3xl shadow-2xl w-[380px] mx-4 overflow-hidden">
        <div class="px-8 pt-8 pb-8 flex flex-col items-center gap-5">
            ...
            {{-- ID Pesanan --}}
            <div class="w-full px-4 py-3 rounded-2xl bg-gray-50 border border-gray-100 flex items-center justify-between">
            ...
            <button onclick="showBatalkanConfirm()" class="w-full py-3.5 rounded-2xl border-2 flex items-center justify-center gap-2 text-sm font-extrabold transition-all hover:bg-red-50" style="border-color:#ef4444; color:#ef4444;">
    ...
    <div class="modal-card bg-white rounded-3xl shadow-2xl w-[380px] mx-4 overflow-hidden">
        <div class="px-8 pt-8 pb-8 flex flex-col items-center gap-4">
            ...
            <div class="flex gap-3 w-full mt-1">
                <button onclick="closeBatalkanConfirm()" class="flex-1 py-3.5 rounded-2xl border-2 border-gray-200 text-sm font-bold text-gray-600 hover:bg-gray-50 transition-all">
                    Kembali
                </button>
                <button onclick="doBatalkan()" id="batalkanBtn" class="flex-1 py-3.5 rounded-2xl text-white text-sm font-extrabold transition-all hover:brightness-110" style="background-color:#ef4444;">
                    Ya, Batalkan
                </button>
            </div>
        </div>
    ...
    <div class="modal-card bg-white rounded-3xl shadow-2xl w-[380px] mx-4 overflow-hidden">
        ...
        <div class="px-8 pt-8 pb-8 flex flex-col items-center gap-4">
            ...
            <div class="flex flex-col gap-2.5 w-full mt-1">
                <a href="/pesanan" class="w-full py-3.5 rounded-2xl text-white font-extrabold text-sm text-center shadow-md" style="background:linear-gradient(135deg,#FF6900,#ea580c);">
                    Lihat Riwayat Pesanan
                </a>
                <a href="/beranda" class="w-full py-3.5 rounded-2xl border-2 border-gray-200 text-gray-600 font-bold text-sm text-center hover:bg-gray-50 transition-all">
                    Kembali ke Beranda
                </a>
            </div>
        </div>
...
<div id="diproseModal" class="fixed inset-0 z-[70] hidden items-center justify-center"
    ...
    <div class="modal-card bg-white rounded-3xl shadow-2xl w-[380px] mx-4 overflow-hidden">
        ...
        <div class="px-8 pt-8 pb-8 flex flex-col items-center gap-4">
            ...
            <div class="flex flex-col gap-2.5 w-full mt-1">
                <button class="w-full py-3.5 rounded-2xl text-white font-extrabold text-sm text-center shadow-md cursor-default" style="background:linear-gradient(135deg,#FF6900,#ea580c);">
                    <i class="fa-solid fa-spinner fa-spin mr-2"></i>
                    Sedang Diproses Admin
                </button>
                <a href="/pesanan" class="w-full py-3.5 rounded-2xl border-2 border-gray-200 text-gray-600 font-bold text-sm text-center hover:bg-gray-50 transition-all">
                    Lihat Pesanan Saya
                </a>
            </div>
        </div>
