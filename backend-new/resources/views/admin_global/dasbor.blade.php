...
<style>
    ...
    .notif-dropdown {
        ...
        border: 1px solid #f3f4f6; z-index: 100; overflow: hidden;
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-8 text-start">
                ...
                <div class="stat-card bg-white p-8 rounded-[40px] border border-gray-100 shadow-sm relative overflow-hidden text-start">
                    <div class="relative z-10 text-start">
                        <p class="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] mb-3">TOTAL PENDAPATAN</p>
                        <h3 class="text-3xl font-black text-gray-900 tracking-tight">
                            Rp {{ number_format($totalPendapatan, 0, ',', '.') }}
                        </h3>
                        <div class="mt-4 flex items-center gap-2">
                            @if($revenueTrend == 'up')
                                <span class="text-[10px] font-black px-2.5 py-1 bg-green-50 text-[#22C55E] rounded-lg">+{{ $revenuePercentage }}%</span>
                            @elseif($revenueTrend == 'down')
                                <span class="text-[10px] font-black px-2.5 py-1 bg-red-50 text-red-500 rounded-lg">-{{ $revenuePercentage }}%</span>
                            @elseif($revenueTrend == 'none')
                                <span class="text-[10px] font-black px-2.5 py-1 bg-gray-50 text-gray-400 rounded-lg">—</span>
                            @else
                                <span class="text-[10px] font-black px-2.5 py-1 bg-gray-50 text-gray-500 rounded-lg">{{ $revenuePercentage }}%</span>
                            @endif
                ...
                <div class="stat-card bg-white p-8 rounded-[40px] border border-gray-100 shadow-sm relative overflow-hidden text-start">
                    <div class="relative z-10 text-start">
                        ...
                        <p class="text-[10px] font-bold text-gray-300 mt-4 italic">Berdasarkan Kantin Aktif Bulan Ini</p>
                    </div>
                </div>
                ...
                <div class="stat-card bg-white p-8 rounded-[40px] border border-gray-100 shadow-sm relative overflow-hidden text-start">
                    <div class="relative z-10 text-start">
                        <p class="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] mb-3">KANTIN AKTIF</p>
                        <div class="flex items-center gap-4">
                            <h3 class="text-3xl font-black text-gray-900 tracking-tight">{{ $kantinAktif }}</h3>
                            <div class="w-10 h-10 bg-green-50 text-[#22C55E] rounded-xl flex items-center justify-center">
                                <i class="fa-solid fa-shop text-lg"></i>
                            </div>
                        </div>
                        <p class="text-[10px] font-bold text-gray-300 mt-4 italic">
